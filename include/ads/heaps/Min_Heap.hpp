 ...
 *
 * @details This class implements a binary min heap where the parent node is always
 *          less than or equal to its children. The heap is stored in a dynamic
...
public:
   ...
   */
  explicit MinHeap(size_type initial_capacity = kInitialCapacity);
  ...
  explicit MinHeap(InputIt first, InputIt last) requires HeapRangeValue<InputIt, T>;
   ...
   * @note Uses bottom-up heapify for O(n) construction.
   */
  MinHeap(std::initializer_list<T> values);
   ...
   * @complexity Time O(log n), Space O(1)
   */
  auto decrease_key(size_type index, const T& new_value) -> void;
