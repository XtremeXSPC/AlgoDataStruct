...
#include "ads/associative/Set.hpp"
...
TEST_F(TreeSetTest, ForEachTraversal) {
  ...
  set.for_each([&values](int val) -> void { values.push_back(val); });
...
TEST(TreeSetMoveOnlyTest, SupportsMoveOnlyValues) {
  ...
  std::vector<int> values;
  set.for_each([&values](const TreeSetMoveOnlyOrdered& value) -> void { values.push_back(value.value); });
