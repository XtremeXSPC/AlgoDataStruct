...
struct MoveOnlyEq {
  ...
  MoveOnlyEq& operator=(MoveOnlyEq&&) noexcept = default;
...
auto collect(Fn traversal) -> std::vector<int> {
  std::vector<int> out;
  traversal([&out](const int& v) { out.push_back(v); });
  return out;
}
...
TEST_F(NaryTreeTest, PreOrderTraversal) {
  build_sample(tree);
  auto seq = collect([&](const auto& v) { tree.pre_order_traversal(v); });
  EXPECT_EQ(seq, (std::vector<int>{1, 2, 5, 6, 3, 4, 7}));
}

TEST_F(NaryTreeTest, PostOrderTraversal) {
  build_sample(tree);
  auto seq = collect([&](const auto& v) { tree.post_order_traversal(v); });
  EXPECT_EQ(seq, (std::vector<int>{5, 6, 2, 3, 7, 4, 1}));
}

TEST_F(NaryTreeTest, LevelOrderTraversal) {
  build_sample(tree);
  auto seq = collect([&](const auto& v) { tree.level_order_traversal(v); });
...
TEST_F(NaryTreeTest, DeepChainTeardownAndHeight) {
  ...
  size_t visited = 0;
  tree.pre_order_traversal([&visited](const int&) { ++visited; });
