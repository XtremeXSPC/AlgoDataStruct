 ...
 * @brief Google Test unit tests for DoublyLinkedList.
 * @version 0.1
 * @date 2025-06-30
 *
...
// Test emplace with a non-trivial type like std::string.
...
TEST_F(DoublyLinkedListTest, ConstCorrectness) {
  ...
  // Verify that iteration over a const list works correctly.
  ...
  // The following code should NOT compile, which proves const_iterator correctness:
  // *const_list.cbegin() = 5;

  // Verify that range-based for loop works with a const list.
...
TEST_F(DoublyLinkedListTest, EraseComprehensive) {
  ...
  EXPECT_EQ(iterator, list.end()); // The returned iterator is end().
  EXPECT_EQ(list.size(), 1);
  EXPECT_EQ(list.back(), 30);
}
...
TEST_F(DoublyLinkedListTest, ReverseIteration) {
  ...
  std::vector<int> rev;
  for (int& it : std::ranges::reverse_view(list)) {
    rev.push_back(it);
  }
  ...
  std::vector<int> crev;
  for (int it : std::ranges::reverse_view(clist)) {
    crev.push_back(it);
  }
...
struct DllMoveOnly {
  ...
  auto operator=(DllMoveOnly&&) noexcept -> DllMoveOnly& = default;
  DllMoveOnly(const DllMoveOnly&)                        = delete;
  auto operator=(const DllMoveOnly&) -> DllMoveOnly&     = delete;
