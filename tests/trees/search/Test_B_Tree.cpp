...
auto expect_matches_set(const BTreeType<int>& tree, const std::set<int>& oracle) -> void {
  ...
  tree.in_order_traversal([&actual](const int& value) -> void { actual.push_back(value); });
...
TEST_F(BTreeTest, IteratorTraversal) {
  ...
  std::vector<int> actual;
  tree.in_order_traversal([&actual](const int& val) -> void { actual.push_back(val); });
...
TEST(BTreeMoveOnlyTest, SupportsMoveOnlyInsertEmplaceAndRemove) {
  ...
  std::vector<int> actual;
  tree.in_order_traversal([&actual](const BTreeMoveOnlyKey& entry) -> void { actual.push_back(entry.key); });
