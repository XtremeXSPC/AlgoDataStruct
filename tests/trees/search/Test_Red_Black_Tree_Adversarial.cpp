...
auto in_order(const RedBlackTree<int>& tree) -> std::vector<int> {
  std::vector<int> out;
  tree.in_order_traversal([&](const int& v) -> void { out.push_back(v); });
  return out;
}
...
auto pre_order(const RedBlackTree<int>& tree) -> std::vector<int> {
  std::vector<int> out;
  tree.pre_order_traversal([&](const int& v) -> void { out.push_back(v); });
  return out;
}
...
auto insert_and_validate_incrementally(RedBlackTree<int>& tree, std::initializer_list<int> values) -> void {
