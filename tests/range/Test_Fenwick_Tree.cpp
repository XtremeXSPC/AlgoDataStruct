 ...
 * @brief Unit tests for the commutative-group FenwickTree implementation.
 * @version 0.1
 * @date 2026-01-28
 *
...
static_assert(FenwickPolicy<int, ads::algebra::AddGroup<int>>);
static_assert(!ads::algebra::CommutativeGroupPolicy<FenwickMissingOps>);
...
TEST(FenwickTreeBasicTest, ConstructionFromInitializerList) {
  ...
  EXPECT_EQ(fenwick.total(), 60);
  EXPECT_EQ(fenwick.prefix_query(1), 30);
}
...
TEST(FenwickTreeBasicTest, ConstructionFromDynamicArrayRange) {
  ...
  EXPECT_EQ(fenwick.size(), 5);
  EXPECT_EQ(fenwick.total(), 14);
  EXPECT_EQ(fenwick.range_query(1, 3), 6);
}

TEST_F(FenwickTreeTest, PrefixSumQueries) {
  EXPECT_EQ(tree.prefix_query(0), 1);
  EXPECT_EQ(tree.prefix_query(2), 6);
  EXPECT_EQ(tree.prefix_query(4), 15);
}
...
TEST_F(FenwickTreeTest, BuildFromDynamicArrayRange) {
  ...
  EXPECT_EQ(tree.prefix_query(1), 5);
  EXPECT_EQ(tree.range_query(1, 3), 13);
}
...
TEST(FenwickTreeEdgeCaseTest, SingleElement) {
  ...
  EXPECT_EQ(fenwick.total(), 42);
  EXPECT_EQ(fenwick.prefix_query(0), 42);
  EXPECT_EQ(fenwick.range_query(0, 0), 42);
  EXPECT_EQ(fenwick.value_at(0), 42);

  fenwick.apply(0, 8);
  EXPECT_EQ(fenwick.value_at(0), 50);
  EXPECT_EQ(fenwick.total(), 50);
}
...
TEST(FenwickTreeEdgeCaseTest, AllZeros) {
  ...
  EXPECT_EQ(fenwick.total(), 0);
  EXPECT_EQ(fenwick.prefix_query(4), 0);
  EXPECT_EQ(fenwick.range_query(1, 3), 0);
...
TEST(FenwickTreeEdgeCaseTest, NegativeValues) {
  ...
  EXPECT_EQ(fenwick.total(), 8);
  EXPECT_EQ(fenwick.prefix_query(0), -5);
  EXPECT_EQ(fenwick.prefix_query(1), 5);
  EXPECT_EQ(fenwick.range_query(0, 2), 2);
  EXPECT_EQ(fenwick.range_query(2, 4), 3);

  fenwick.apply(0, -10);
  EXPECT_EQ(fenwick.value_at(0), -15);
  EXPECT_EQ(fenwick.total(), -2);
}

TEST(FenwickTreeEdgeCaseTest, LargeValues) {
  FenwickTree<long long> fenwick{1'000'000'000LL, 2'000'000'000LL, 3'000'000'000LL};
  EXPECT_EQ(fenwick.total(), 6'000'000'000LL);
  EXPECT_EQ(fenwick.prefix_query(1), 3'000'000'000LL);
}
...
TEST(FenwickTreeCustomTypeTest, DoubleType) {
  ...
  EXPECT_DOUBLE_EQ(fenwick.total(), 7.0);
  EXPECT_DOUBLE_EQ(fenwick.prefix_query(1), 4.0);
  EXPECT_DOUBLE_EQ(fenwick.range_query(0, 1), 4.0);

  fenwick.apply(0, 0.5);
  EXPECT_DOUBLE_EQ(fenwick.value_at(0), 2.0);
  EXPECT_DOUBLE_EQ(fenwick.total(), 7.5);
}

TEST(FenwickTreeCustomTypeTest, LongLongType) {
  FenwickTree<long long> fenwick{1LL, 2LL, 3LL, 4LL, 5LL};
  EXPECT_EQ(fenwick.total(), 15LL);
  EXPECT_EQ(fenwick.prefix_query(2), 6LL);

  fenwick.set(0, 100LL);
  EXPECT_EQ(fenwick.value_at(0), 100LL);
  EXPECT_EQ(fenwick.total(), 114LL);
}
...
TEST(FenwickTreeErrorTest, OutOfRangeThrows) {
  ...
  EXPECT_THROW(fenwick.apply(3, 1), RangeIndexException);
  EXPECT_THROW({ [[maybe_unused]] auto _ = fenwick.prefix_query(3); }, RangeIndexException);
  EXPECT_THROW({ [[maybe_unused]] auto _ = fenwick.range_query(2, 1); }, InvalidRangeException);
  EXPECT_THROW({ [[maybe_unused]] auto _ = fenwick.range_query(0, 3); }, InvalidRangeException);
}

TEST(FenwickTreeErrorTest, ValueAtOutOfRange) {
  FenwickTree<int> fenwick{1, 2, 3};
  EXPECT_THROW({ [[maybe_unused]] auto _ = fenwick.value_at(3); }, RangeIndexException);
  EXPECT_THROW({ [[maybe_unused]] auto _ = fenwick.value_at(100); }, RangeIndexException);
}

TEST(FenwickTreeErrorTest, SetOutOfRange) {
  FenwickTree<int> fenwick{1, 2, 3};
  EXPECT_THROW(fenwick.set(3, 10), RangeIndexException);
}
