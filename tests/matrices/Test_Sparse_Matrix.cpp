...
TEST(SparseMatrixConstructionTest, BuildsFromTripletsInRowMajorOrder) {
  SparseMatrix<int> matrix(
      4, 5,
      {
          {.row = 2, .column = 4, .value = 9},
          {.row = 0, .column = 1, .value = 7},
          {.row = 2, .column = 1, .value = 3},
          {.row = 3, .column = 0, .value = 5},
      });
...
TEST(SparseMatrixAccessTest, AtThrowsForStructuralZeros) {
  SparseMatrix<int> matrix(2, 3, {{.row = 0, .column = 2, .value = 8}});
...
TEST(SparseMatrixMutationTest, RowIterationRemainsColumnSortedAfterMutations) {
  ...
  matrix.for_each_in_row(1, [&](size_t column, const int& value) -> void { visited.emplace_back(column, value); });
...
TEST(SparseMatrixIterationTest, EnumeratesNonZeroEntriesInRowMajorOrder) {
  SparseMatrix<int> matrix(
      3, 4,
      {
          {.row = 2, .column = 0, .value = 9},
          {.row = 0, .column = 2, .value = 5},
          {.row = 1, .column = 1, .value = 4},
      });

  std::vector<std::tuple<size_t, size_t, int>> visited;
  matrix.for_each_non_zero([&](size_t row, size_t column, const int& value) -> void { visited.emplace_back(row, column, value); });
...
TEST(SparseMatrixUtilityTest, ClearPreservesShapeAndRemovesEntries) {
  SparseMatrix<int> matrix(5, 6, {{.row = 1, .column = 2, .value = 9}, {.row = 4, .column = 5, .value = 7}});
