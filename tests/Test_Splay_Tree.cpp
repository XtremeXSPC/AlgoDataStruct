...
TEST_F(SplayTreeTest, InOrderIsSorted) {
  ...
  tree.in_order_traversal([&collected](const int& v) { collected.push_back(v); });
...
TEST_F(SplayTreeTest, PreOrderPostOrderLevelOrderVisitAllNodes) {
  ...
  size_t post  = 0;
  size_t level = 0;
  tree.pre_order_traversal([&pre](const int&) { ++pre; });
  tree.post_order_traversal([&post](const int&) { ++post; });
  tree.level_order_traversal([&level](const int&) { ++level; });
