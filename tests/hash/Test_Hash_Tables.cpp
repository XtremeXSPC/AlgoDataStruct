...
template <typename Map, typename Key, typename Value> concept HasHashMapCopyPut = requires(Map& map, const Key& key, const Value& value) {
...
static_assert(ads::associative::Dictionary<ads::associative::HashMap<int, std::unique_ptr<int>>, int, std::unique_ptr<int>>);
