 ...
 * @brief Demonstrates monoid-driven Segment Tree queries and updates.
 * @version 0.2
 * @date 2026-02-04
 *
 * @copyright MIT License 2026
 *
 * This program demonstrates the algebra-driven SegmentTree API, showcasing
 * range queries, point updates, and alternative aggregation monoids.
 */
//===---------------------------------------------------------------------------===//
...
auto demo_build_and_queries() -> void {
  ...
  cout << "Sum [2..4]: " << tree.range_query(2, 4) << "\n";
}
...
//===----- BUILT-IN MONOID DEMO ------------------------------------------------===//

// Demonstrates a maximum Segment Tree using the shared algebra policy.
auto demo_max_monoid() -> void {
  ads::demo::print_section("Built-in Max Monoid");

  vector<int>                                    values = {1, 3, -2, 8, -7};
  SegmentTree<int, ads::algebra::MaxMonoid<int>> tree(values);
...
auto demo_custom_nodes() -> void {
  ...
  SegmentTree<int, SumCountMonoid, SumCountLeaf> tree(values);
